...
--○ 테이블 생성(TBL_EXAMPLE2)
CREATE TABLE TBL_EXCAMPLE2
( NO    NUMBER
, NAME  VARCHAR2(10)
, ADDR  VARCHAR2(20)
)TABLESPACE TBS_EDUA;
--==> Table TBL_EXCAMPLE2이(가) 생성되었습니다.
...
/*
BONUS	        TABLE	    → 보너스 테이블
DEPT	        TABLE	    → DEARTMENTS(부서) 테이블
EMP	            TABLE	    → EMPLOYEES(사원) 테이블
SALGRADE	    TABLE	    → 급여(SAL)의 등급
TBL_EXAMPLE1	TABLE	    
TBL_EXCAMPLE2	TABLE	     
*/
...
-- ※ 우리가 흔히 웹 사이트에서 회우너 가입을 수행할 경우
--    필수 입력 사항과 선택 입력 사항이 있다.
--    필수 입력 항목은 ID, PW, 성명, 주민번호, 전화번호, 주소, ...
--    등과 같은 컬럼이며, 이 값들ㅇ느 회원가입 절차에 따라
--    반드시 필요한(존재해야 하는, 입력해야하는) 값이므로 NOT NULL로 한다.

-- 선택 입력 항목은 취미,결혼 여부, 차량소유여부,특ㅂ기, ...
...
--※ ORACLE서버는 숫자 표현 타입이 한가지로 통일되어 있다.
/*
...
2. 문자형 CHAR         - 고정형 크기
          ...
          CHAR(10) ← 'OH집중서승균'  10Byte를 초과하므로 입력 불가
        
          VARCHAR2     - 가변형 크기
          VARCHAR2(10) ← '강의실'  6Byte
          VARCHAR2(10) ← '잠깬서승균' 10Byte
          VARCHAR2(10) ← 'OH집중서승균'  12Byte이므로 10Byte를 초과하므로 입력 불가
